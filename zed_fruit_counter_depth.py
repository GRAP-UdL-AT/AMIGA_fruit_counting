...
def main():
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            depth = mat_depth.get_data()
            #depth_image = (depth*255)/4000
            #depth_image = depth_image.astype(np.uint8)

            #cv2.imshow("hola", depth_image)
            frame = mat.get_data()
            ...
            #Function used to filter detections by depth
            new_detections = complementary.filter_by_depth(dets_conf, new_detections, depth, frame)
            '''
            for detection in dets_conf:
                x_center = (detection[0]+detection[2])/2
                y_center = (detection[1]+detection[3])/2

                bbox_center = (int(x_center), int(y_center))
                bbox_depth = depth[int(detection[1]):int(detection[3]),int(detection[0]):int(detection[2])]
                bbox_img = frame[int(detection[1]):int(detection[3]),int(detection[0]):int(detection[2]),:]

                #cv2.imshow("hola", bbox_img)
                mean_depth = np.mean(bbox_depth)

                #if not depth[bbox_center] > 1000:
                if not mean_depth > 1000:
                    new_detections.append(detection)
                #cv2.circle(frame, bbox_center, 2, (255, 0, 0), 2)
            ...
            fruit_tracker.get_detections(new_detections, i, frame)
